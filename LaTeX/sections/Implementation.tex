...
The red box in figure \ref{Pitch_expl} shows the part of the patch that indicates how big the increments in shifting should be. To raise the pitch by one octave, the playback speed has to be doubled. Knowing that an octave consists of 12 halftones it means that to raise a pitch by a halftone the playback speed should be raised by 2^(1/12). The prototype was made to pitch shift 3 halftones at a time, meaning that the playback speed has to be raised by 2^(3/12) which equals 1.189 for each pitch shift.
\begin{minipage}{\linewidth}% to keep image and caption on one page
\makebox[\linewidth]{%        to center the image
...
The "tapes" speed is 1 so to make the pitch shift, one of the read points scans
the sound, delayed but at a faster rate than the "tape" is playing, so that the delay progressively goes to 0. To support this the sawtooth signal is inverted allowing for the delay to start at maximum values and to go to minimal values before resetting back to the maximum values. This is shown in the yellow box on figure \ref{Pitch_expl}.

The green box on figure \ref{Pitch_expl} illustrates the read point objects, which each scan 20 frames of the signal per second, which means that each frame is 50 ms long.
